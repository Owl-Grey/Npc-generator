...
class Pop(Popup):
    ...
    def set_description(self, title):
        ...
        if len(pr_arr[i])>5:
            ...
            self.ids.bsnm.add_widget(Label(text="Противник",font_size='100',bold=True,italic=True))
############################################
class InsPanel(TabbedPanel):
    ...
    def rand_wear(self):
        mn=""
        l=randint(0,len(wear)-2)
        if l==0:
            mn+=str(choice(wear[l]))+"\n\n"
            mn+=str(choice(wear[3]))
        if l==1:
            mn+=str(choice(wear[l]))+"\n"
            mn+=str(choice(wear[l+1]))+"\n\n"
            mn+=str(choice(wear[len(wear)-1]))
        if l==2:
            mn+=str(choice(wear[l-1]))+"\n"
            mn+=str(choice(wear[l]))+"\n\n"
            mn+=str(choice(wear[len(wear)-1]))
            ...
            for i in range(25):
                ...
                if x==1:
                    ...
                    if sus[z][k-1] == "а" or sus[z][k-1] == "ь" or sus[z][k-1] == "я":
                        ...
                        l= j + " " + str(gun_corps[x]) + ' "' + str(pril[y]) + " " + str(sus[z]) + '"'+"\n"+ ur
                guns.append(l)
    def rand_chmot(self, *args):
        for num in range(len(wear)):
            wear[num].clear()
